...
// Send ONE packet per call (called repeatedly from main loop)
// Returns: 1 = packet sent successfully, 0 = buffer full (retry later), -1 = error
...
static int send_next_packet(void){
	...
	if (g_mic_idx >= MIC_COUNT) {
		// Frame complete, advance to next frame
		g_mic_idx = 0;
		g_frame_no++;
		g_tcp_output_calls++;
	...
	if (avail < PACKET_BYTES){
		g_snd_buf_stalls++;
		g_immediate_stalls++;
		return 0;  // Buffer full - can't send more packets this loop iteration
	}
	...
	// Don't flush on every packet - let it build up
	// Only flush when frame complete (in stream_scheduler_run)
...
void stream_scheduler_run(void)
{
	...
	// If mid-frame, keep trying to complete it
	if (g_mic_idx > 0) {
		send_next_packet();
		// Don't return - this allows main loop to call other functions
	}

	// Send next frame when data is available and previous frame complete
	if (g_mic_idx == 0 && frame_data_ready()) {
		g_last_frame_ms = g_ms;
		
		// Debug: First transmission and every 100 frames
		static int first_tx = 1;
		if (first_tx) {
			xil_printf("[TX] Starting first frame transmission\r\n");
			first_tx = 0;
		} else if ((g_frame_no % 100) == 0) {
			xil_printf("[TX] Starting frame %u transmission\r\n", g_frame_no);
		}
		
		// Begin new frame by sending first packet (data already deinterleaved in RAM)
		int result = send_next_packet();
		if (result == 1 && g_mic_idx >= MIC_COUNT) {
			// Frame complete
#if USE_DDR_READS
			dma_frame_processed();
			if ((g_frame_no % 100) == 0) {

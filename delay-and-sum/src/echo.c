...
static err_t data_connected(void *arg, struct tcp_pcb *tpcb, err_t err){
	...
	xil_printf("[DATA] Waiting for frame ready signal...\r\n");
...
#if USE_DDR_READS
static volatile uint16_t *g_current_frame_base = NULL;
...
#if USE_DDR_READS
// DMA buffer layout: 128 channels interleaved (102 active mics + padding)
#define NUM_CHANNELS 128

// Pre-deinterleaved frame data (allocated in RAM for fast access)
// 102 mics × 512 samples × 2 bytes = ~104 KB
static int16_t g_deinterleaved_data[MIC_COUNT][SAMPLES_PER_MIC];
static volatile int g_frame_deinterleaved = 0;  // 1 = data ready

// Deinterleave entire frame at once (called when DMA interrupt fires)
// Sequential read through DDR is ~10x faster than 102 strided reads
static void deinterleave_frame(volatile uint16_t *frame_base)
{
    if (frame_base == NULL) {
    ...
    xil_printf("[DEINTERLEAVE] Frame processed, data ready in RAM\r\n");
}

// Get pre-deinterleaved samples for one mic (just a fast memcpy)
static void read_mic_samples_from_ddr(uint16_t mic_id, int16_t *out_samples)
{
    if (!g_frame_deinterleaved) {
        ...
        generate_samples(mic_id, out_samples);  // Fallback
        return;
    }
    
    // Fast copy from pre-deinterleaved RAM buffer
    memcpy(out_samples, g_deinterleaved_data[mic_id], SAMPLES_PER_MIC * sizeof(int16_t));
}
#endif
...
static int send_next_packet(void){
	if (!data_pcb) {
		xil_printf("[TX_DEBUG] No PCB!\r\n");
		return 0;
	}
	...
	if (first_packet) {
		xil_printf("[TX_DEBUG] Sending first packet (mic %u, frame %u)\r\n", g_mic_idx, g_frame_no);
		first_packet = 0;
	}
	...
	if (avail < PACKET_BYTES){
		g_snd_buf_stalls++;
		g_immediate_stalls++;
		if (g_mic_idx == 0) {  // Only print for first mic to avoid spam
			xil_printf("[TX_DEBUG] Buffer full: avail=%u, need=%u\r\n", avail, PACKET_BYTES);
		}
	...
	if (e != ERR_OK) {
		xil_printf("[TX_ERROR] tcp_write failed: err=%d, mic=%u\r\n", e, g_mic_idx);
		g_tcp_write_errors++;
		g_immediate_stalls++;
	...
	if (g_mic_idx < 3) {  // First 3 packets
	...
	// Flush buffer every 5 packets to prevent buffer exhaustion
	// This allows ACKs to return and free send buffer space
	if ((g_mic_idx % 5) == 0) {
		tcp_output(data_pcb);
	}
	
	// Debug mic index progression
	if (g_mic_idx == 1 || g_mic_idx == 50 || g_mic_idx == 101) {
		xil_printf("[TX_PROGRESS] Now at mic %u/%u\r\n", g_mic_idx, MIC_COUNT);
	}
...
#if USE_DDR_READS
		// Notify DMA we're done with this frame buffer
		dma_frame_processed();
		// Print every 10th frame to reduce console spam
		if ((g_frame_no % 10) == 0) {
			xil_printf("[TX] Frame %u complete (102 packets sent)\r\n", g_frame_no-1);
		}
#else
		if ((g_frame_no % 10) == 0) {
			xil_printf("[TX] Frame %u complete (test data)\r\n", g_frame_no-1);
		}
#endif
...
void stream_scheduler_run(void)
{
	...
	// On new frame: get buffer and deinterleave entire frame at once
	if (g_mic_idx == 0 && frame_ready()) {
#if USE_DDR_READS
		g_current_frame_base = (volatile uint16_t *)dma_get_current_read_buffer();
		
		// Print first frame detection
		static int first_frame_detected = 0;
		if (!first_frame_detected) {
			xil_printf("\r\n[FRAME] *** FIRST FRAME DETECTED! ***\r\n");
			xil_printf("[FRAME] Buffer at 0x%08X\r\n", (uint32_t)g_current_frame_base);
			xil_printf("[FRAME] Starting transmission...\r\n\r\n");
			first_frame_detected = 1;
		}
		
		// Deinterleave entire frame NOW (sequential DDR read, cache-efficient)
		deinterleave_frame(g_current_frame_base);
		
		// Clear frame flag now that we've processed it
		frame_clear();
#endif
	}

	// Send one packet per call (paced by 100us timer)
	// Keep calling until all 102 mics sent
	if (g_mic_idx < MIC_COUNT) {
		send_next_packet();
	}
}
